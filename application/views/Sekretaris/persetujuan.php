    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
              ...
              <h3 class="card-title">Persetujuan Persuratan</h3>
            </div>
            <div class="card-body">
              ...
              ?>
                <tr>
                  ...
                  <td class="text-center">
                    <a href="<?= site_url('ketum/download/surat_proker/') . $row->lampiran ?>" target="_blank" class="btn btn-primary btn-xs">Lihat File</a>
                  </td>
                  <td>
                    <?php if ($row->status == 1 || $row->status == 3 || $row->status == 4|| $row->status == 5|| $row->status == 6) { ?>
                  ...
                  <td class="text-center">
                    <?php if ($row->status == 0) { ?>
                      <button class="btn btn-primary btn-xs" data-toggle="modal" data-target="#setuju" onclick="fetchSetuju(this)" data-id_dokumen="<?= $row->id_dokumen ?>" data-tanggal="<?= $row->tanggal ?>" data-perihal="<?= $row->perihal ?>" data-tujuan="<?= $row->keterangan ?>" data-lampiran="<?= $row->lampiran ?>"><i class=""></i>Beri No. Surat</button>
                      <button class="btn btn-danger btn-xs" data-toggle="modal" data-target="#tolak" onclick="fetchTolak(this)" data-id_dokumen="<?= $row->id_dokumen ?>">Tolak</button>

                    <?php } elseif ($row->status == 1 || $row->status == 2 || $row->status == 3 || $row->status == 4 || $row->status == 5 || $row->status == 6) { ?>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header" style="background-color: dodgerblue; color: whitesmoke;">
        <h5 class="modal-title">Beri No. Surat</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span style="color: whitesmoke;" aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form role="form" action="<?php echo site_url('Sekretaris/persetujuan') ?>" method="post" enctype="multipart/form-data" class="form-submit">

          <div class="form-group">
                        <label for="exampleInputFile">Uploud File Yang Di Setujui</label>
                       <div class="form-group">
